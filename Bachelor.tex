...
Dann wird ein Überblick über die verschiedenen Mechanismen der Photoemission gegeben, bevor die Multiphoton-Photoemission mit Hilfe der Fowler-DuBridge-Theorie hergeleitet wird.
...
e
	...
	Allerdings trifft dies nur für kleine Intensitäten zu, da im Rahmen einer störungstheoretischen Betrachtung des Emissionsprozesses gezeigt werden kann, dass für große Intensitäten des eingestrahlten optischen Feldes die Wahrscheinlichkeit mit $I^N$ ansteigt, dass $N$ Photonen <+annähernd gleichzeitig+> auftreffen und wechselwirken.
	Dann steht die Summe der Einzelenergien zur Verfügung.
...
%	Im Experiment lässt sich $N$ durch die Variation der eingestrahlten Lichtintensität bei gleichzeitiger Messung des Elektronenstroms bestimmen.
	...
	\item \textit{Above Threshhold Ionisation}
		<+Ist dies nur bei Multiphoton PE möglich oder tritt das auch bei Feldemission auf?+>
...
Im folgenden wird genauer auf die Theorie hinter der Multiphoton-PES eingegangen.

\section{Fowler-DuBridge-Theorie der Multiphoton-Photoemission}
Die Multiphoton-PES kann durch die \textsc{Fowler-DuBridge}-Theorie beschrieben werden\cite{bechtel_1977}\cite{girardeau_1995}.
...
Dies lässt sich ebenfalls aus einer störungstheoretischen Betrachtung herleiten.
Dabei wird das Atompotential $H_0$ durch das zeitabhängige Potential des elektrischen Feldes erweitert, wodurch die Schrödingergleichung für die Wellenfunktion $\psi$
\begin{align}
	i\hbar\pdv{}{t}\Psi(t)=[H_0+ V(t)]\Psi(t)
	\label{eq:schroedinger}
\end{align}
lautet.
Gilt $V(t)\rightarrow0$ für $t\rightarrow\pm\infty$, so kann man mit
\begin{align*}
...
Für die Energie-Eigenzustände $\ket i$ des ungestörten Atompotentials gilt dabei $$H_0\ket i=\hbar\omega_i\ket i$$
Was als bekannt voraus gesetzt wird.
Eine Lösung für Gleigung \ref{eq:schroedinger_kompakt} ist
\begin{align}
	\psi_i=\ket i+\left( -i/\hbar \right)\int\limits_{-\infty}^tV(t')\ket{\psi_i(t')}dt'	\label{eq:psi_i}
\end{align}
...
Ist der Endzustand $\ket f$, so ist die Übergangswahrscheinlichkeit von $\ket i$ nach $\ket f$ natürlich $\braket{f|\psi_i(t)}$ oder
\begin{align}
	\braket{f|\psi_i(t)}=\bra fA(t)\ket i \label{eq:uebergangswahrscheinlichkeit_1}
\end{align}
mit dem Amplitudenoperator $A(t)$.
Dies kann auch durch
\begin{align}
	\ket{\psi_i(t)}=A(t)\ket i
	\label{eq:psiA}
\end{align}
geschrieben werden, da $\ket f$ unabhängig ist.
...
% Der Hamiltonian der Wechselwirkung von Elektron und elektromagnetischem Feld lautet nach \cite[S. 130]{sakurai}
% \begin{align}
% 	\begin{split}
% 	H'&=\frac{1}{2m}\left(\boldsymbol p-\frac{e\bold A}{c} \right)^2+e\phi\\\
% 	&=\frac{p^2}{2m}-\frac{e}{2mc}\left( \boldsymbol p \cdot\bold A+\bold A\cdot\boldsymbol p \right)+\frac{e^2}{2mc^2}\bold A^2+e\phi\qquad .
% 	\label{eq:kopplung}
% \end{split}
% \end{align}
% Dabei bezeichnet $\boldsymbol p=-i\hbar\nabla$ den kanonischen Impuls-Operator, $\bold A$ das Vektorpotential des Magnetfeldes ($\bold B=\nabla\times \bold A$), $\phi$ das Potential des elektrischen Feldes ($\bold E=-\nabla\phi$).\\\\
...
\label{sec:schottky}
Der sogenannte \textsc{Schottky}-Effekt erm\"oglicht die Verringerung der Austrittsarbeit durch das Anlegen einer Spannung $U$.
Dies wird in der schematischen Abbildung \ref{fig:schottky} dargestellt.
Dies ist m\"oglich, da sich Elektron und die induzierte Sipegelladung durch ihr unterschiedliches Vorzeichen in der Ladung anziehen (siehe unverändertes Potential in schwarz mit $U=\SI0\V$).
...
In der Literatur wird meist $k\approx5$ angenommen (z.B. \cite{}).
Für die hier verwendeten Parameter Spitzenradius $r=\SI{45}{\nm}$ und angelegte Spannung $U_\text{Tip}=\SI{100}{\V}$ ergibt sich eine Feldstärke von  $F=\SI{0.35}{\V\per\nm}$.
...
\item Helligkeit integrieren: Die Helligkeit der einzelnen Pixel \"uber das ganze Bild summieren.
	Vorteilhaft an dieser Methode ist, dass gerade bei hohen Intensit\"aten der Algorithmus zuverl\"assiger ist.
	Sollten die Elektronen jedoch nicht vergleichbar in der Helligkeit sein (z.B. da die Kamera keine lineare Intensit\"atsskala besitzt),  gibt der Algorithmus eine falsche Anzahl heraus.
	Insbesondere kleine Anzahlen an Elektronen in einem Bild kann diese Methode nur schlecht ermitteln.
\end{itemize}

Da die verwendeten Bilder nur eine relativ geringe Anzahl an Elektronen besa{\ss}en, wurde das erste Programm zur Auswertung gew\"ahlt.
...
Einige Punkte, welche man auf dem Bild erkennt nicht detektiert wurden, liegt daran, dass die Farbskala sehr eingeschr\"ankt ist und die Punkte nur sehr schwach sind.
Die blau umrandeten beispielsweise sind nur wenige Pixel im Durchmesser und k\"onnen so auch durch Rauschen hervorgerufen worden sein und nur f\"ur das Auge dominant wirken.
...
Da jedoch nicht viele Leute an dem Lasersystem vorbeigingen und im späteren Verlauf der Messungen darauf geachtet wurde, dass w\"ahrend der Messungen kein Luftzug entstand, sind höchstens einzelne Datenpunkte von einzelnen Wellenlängen betroffen.
